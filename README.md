...
8. 訂單
   ...
   4. 交易對象:長度為10的字串, 參考 `客戶之客戶ID`, 不可為空。
...
| 實體 | 屬性     | Domain                                 | 說明                                                                 |
| ---- | -------- | -------------------------------------- | -------------------------------------------------------------------- |
| 銀行 | 銀行代碼 | 格式為 `金融機構總代號` `分支機構代號` | 客戶付款的銀行帳號代碼                                               |
|      |          | `^([0-9]{3}\|[0-9]{7})$`               | 通常只需`金融機構總代號`，在填入`分支機構代號`時，需確認是否有總代號 |
...
|      | 客戶名稱 | 長度為40的字串         | 考慮可能為個人與法人(公司)而設定此長度                   |
...
| 實體 | 屬性     | Domain                                              | 說明                         |
| ---- | -------- | --------------------------------------------------- | ---------------------------- |
| 倉庫 | 倉庫ID   | 格式為 `WH-1000`                                    | 每間倉庫由程式隨機分配ID     |
|      |          | `^WH-[0-9]{4}$`                                     | 此格式可記錄9999個倉庫       |
...
| 實體       | 屬性     | Domain | 說明                                                                               |
| ---------- | -------- | ------ | ---------------------------------------------------------------------------------- |
| 收付款明細 | 帳單編號 |        | 紀錄收付款明細的，此編號由系統產生                                               |
|            | 訂單編號 |        | 銷售或進貨訂單上的訂單編號，此編號由系統產生                                     |
|            | 銀行名稱 |        | 對應銀行代碼之銀行名稱                                                           |
|            | 交易對象 |        | 交易對象可為客戶時，指此為銷售為本公司時，則為進貨此欄位參考修交易的交易對象 |
...
2. 出貨關係
   ...
   |      |        |        |      |
    ...
    CREATE TABLE ItemOrderDetails (
      ItemID INT,
      OrderDetailID INT,
      PRIMARY KEY (ItemID, OrderDetailID),
      FOREIGN KEY (ItemID) REFERENCES Items(ItemID),
      FOREIGN KEY (OrderDetailID) REFERENCES OrderDetails(OrderDetailID)
    );
    ```

    > 是的，ItemOrderDetails 關聯表是必要的。在您提供的情況下，由於物品和訂單明細之間存在多對多的關係，單獨的物品表或訂單明細表無法有效地存儲這種關係的所有信息。關聯表允許每個物品與多個訂單明細相關聯，反之亦然。
    >
    > 在沒有 ItemOrderDetails 關聯表的情況下，您將無法準確地表示哪些物品屬於哪些訂單明細，特別是當一個物品可以出現在多個訂單明細中，或者一個訂單明細可以包含多個不同的物品時。因此，創建一個關聯表是實現多對多數據庫關係的標準做法。
