...
1. 物品ID:長度為15的字串
   1. 前五碼為大寫英文AAAAA~ZZZZZ
   2. 後十碼為正整數0000000000~9999999999。
...
1. 帳單編號:長度為10的字母與數字,不可為空。格式如下。
   ...
   2. 第4碼為字元"-"
   3. 後6碼為隨機的數字
2. 訂單編號:長度為12的字串,不可為空,格式如前述。
3. 交易對象:長度為50的中文字串,不可為空
4. 銀行代碼:長度為3的字串,不可為空
5. 銀行名稱:長度為50字串
6. 銀行帳號:長度為14的字串,7+7(局號+帳號),不可為空
7. 交易日期:儲存日期時間,不得小於從公司建立日期
8. 金額:正整數,不可為空
...
1. 訂單編號:長度為12的字串,不可為空,格式如下。
   ...
   3. 後3碼為隨機的英文字母及數字
2. 物品ID:長度為15的字串。
3. 員工ID:長度為10的字串,不可為空,格式為DEP-POS-ID。
4. 交易類別:長度為2的字串,僅為"購入" 或"銷售",不可為空。
5. 付款日期:儲存日期時間,可為空,不得小於從公司建立日期,預設值為NULL。
6. 付款方式:長度為10的中文字串,可為空,預設值為NULL。
7. 交易對象:長度為50的中文字串,不可為空。
